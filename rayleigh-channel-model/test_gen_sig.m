...
x=[0:N-1]*Ts;
initial_signal = 12*sin(50*x)+2*cos(500*x);
...
quantised = filtered_signal / mean(filtered_signal) < 1;
...
figure;
plot([0:length(crossings_up)-1]*Ts,crossings_up);

% autocorrelation 

acf = xcorr(filtered_signal,filtered_signal);
